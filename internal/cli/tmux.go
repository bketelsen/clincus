...
The session name should be the container name (e.g., clincus-abc123-1).`,
...
func tmuxSendCommand(cmd *cobra.Command, args []string) error {
	...
	tmuxSession := fmt.Sprintf("clincus-%s", containerName)
...
func tmuxListCommand(cmd *cobra.Command, args []string) error {
	...
	containers, err := container.ListContainers("clincus-.*")
	if err != nil {
	...
	for _, c := range containers {
		...
		tmuxSession := fmt.Sprintf("clincus-%s", c)
