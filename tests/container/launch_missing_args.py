"""
Test for clincus container launch - fails with missing arguments.

Tests that:
1. Run clincus container launch without required arguments
...
def test_launch_missing_args(clincus_binary, cleanup_containers):
    """
    Test that launch without required arguments shows usage.

    Flow:
    1. Run clincus container launch with no arguments
    2. Verify it fails
    ...
    result = subprocess.run(
        [clincus_binary, "container", "launch"],
        capture_output=True,
        text=True,
        timeout=30,
    )
