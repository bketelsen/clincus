"""
Test that incus config values from .clincus.toml are wired into command execution.
...
Tests that:
1. Custom code_uid from .clincus.toml is used when executing commands in containers
...
def test_custom_code_uid_from_config(clincus_binary, cleanup_containers, workspace_dir):
    """
    Test that incus.code_uid from .clincus.toml is applied to container exec.

    Flow:
    1. Create .clincus.toml with code_uid = 1001
    2. Run clincus run "id -u"
    3. Verify UID is 1001 (not the default 1000)
    """
    # Create .clincus.toml with custom code_uid
...
"""
    ...
    # Run from workspace_dir so config.Load() finds the .clincus.toml
...
def test_default_code_uid_without_config(clincus_binary, cleanup_containers, workspace_dir):
    """
    Test that default code_uid (1000) is used when no config override is present.

    Flow:
    1. Run clincus run "id -u" without any .clincus.toml
