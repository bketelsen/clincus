...
def test_build_custom_simple(clincus_binary, tmp_path):
    """Test building a custom image with a simple script."""
    image_name = "clincus-test-custom-simple"
...
""")
    ...
    # Build custom image (skip if clincus-sandbox doesn't exist)
    ...
    # Verify image exists
    result = subprocess.run(
        [clincus_binary, "image", "exists", image_name],
        capture_output=True,
    )
    assert result.returncode == 0, "Custom image should exist"

    # Launch container from custom image to verify
    container_name = "clincus-test-custom-verify"
    result = subprocess.run(
        [clincus_binary, "container", "launch", image_name, container_name],
        capture_output=True,
        text=True,
    )
    ...
    result = subprocess.run(
        [clincus_binary, "container", "exec", container_name, "--", "which", "curl"],
        capture_output=True,
        text=True,
    )
    ...
    # Cleanup
    subprocess.run([clincus_binary, "container", "delete", container_name, "--force"], check=False)
