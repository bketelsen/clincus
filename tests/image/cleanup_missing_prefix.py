"""
Test for clincus image cleanup - missing prefix argument.

Tests that:
1. Run clincus image cleanup without prefix
...
def test_cleanup_missing_prefix(clincus_binary, cleanup_containers):
    """
    Test that cleanup without prefix argument fails.

    Flow:
    1. Run clincus image cleanup --keep 1 (no prefix)
    2. Verify it fails with usage message
    """
    result = subprocess.run(
        [clincus_binary, "image", "cleanup", "--keep", "1"],
        capture_output=True,
        text=True,
        timeout=30,
    )
