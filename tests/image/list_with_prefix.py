"""
Test for clincus image list --prefix - filter images by prefix.

Tests that:
1. Run clincus image list --prefix <prefix>
...
def test_list_with_prefix(clincus_binary, cleanup_containers):
    """
    Test listing images filtered by prefix.

    Flow:
    1. Run clincus image list --prefix clincus
    2. Verify output only shows matching images or "no images found"
    """
    # === Phase 1: Run image list with prefix ===

    result = subprocess.run(
        [clincus_binary, "image", "list", "--prefix", "clincus"],
        capture_output=True,
        text=True,
        timeout=30,
    )
    ...
    assert "clincus" in combined_output.lower() or "No images found" in combined_output, (
        f"Should show filtered results or 'No images found'. Got:\n{combined_output}"
    )


def test_list_with_nonexistent_prefix(clincus_binary, cleanup_containers):
    """
    Test listing images with prefix that matches nothing.

    Flow:
    1. Run clincus image list --prefix nonexistent-xyz-123
    2. Verify it shows "no images found" message
    """
    # === Phase 1: Run image list with nonexistent prefix ===

    result = subprocess.run(
        [clincus_binary, "image", "list", "--prefix", "nonexistent-xyz-123-abc"],
        capture_output=True,
        text=True,
        timeout=30,
    )
