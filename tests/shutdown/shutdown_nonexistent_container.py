"""
Test for clincus shutdown - nonexistent container.
...
def test_shutdown_nonexistent_container(clincus_binary, cleanup_containers):
    """
    Test shutting down a container that doesn't exist.

    Flow:
    1. Run clincus shutdown nonexistent-container-xyz
    2. Verify it fails or shows warning
    """
    result = subprocess.run(
        [clincus_binary, "shutdown", "nonexistent-container-xyz-123"],
        capture_output=True,
        text=True,
        timeout=30,
    )
